...
std::ostream& stream_string(std::ostream& stream,
...
std::ostream& operator<<(std::ostream& stream, const SRL::JSON::Value& v) {
    if (v.is<double>()) {
    ...
    } else if (v.is<SRL::JSON::Value::Null>()) {
        return stream << "null";
    } else if (v.is<SRL::JSON::Object>()) {
        return stream << v.get<SRL::JSON::Object>();
    } else if (v.is<SRL::JSON::Array>()){
        return stream << v.get<SRL::JSON::Array>();
    }
    // Shouldn't reach here.
    return stream;
}

std::ostream& operator<<(std::ostream& stream, const SRL::JSON::Array& v) {
    stream << "[";
    const std::vector<SRL::JSON::Value*>& values(v.values());
...
std::ostream& operator<<(std::ostream& stream, const SRL::JSON::Object& v) {
    stream << "{";
    const std::map<std::string, SRL::JSON::Value*>& kv(v.kv_map());
    for (std::map<std::string, SRL::JSON::Value*>::const_iterator i = kv.begin();
         i != kv.end(); /**/) {
        stream_string(stream, i->first);
        stream << ": " << *(i->second);
        ++i;
        if ( i != kv.end()) {
            stream << ", ";
